...
def get_df_final(save_csv=False):
    load_dotenv()
    ...
    if all_rows:
        ...
        # Function to split and clean values
        def separar_noms(x):
            if pd.isna(x):
                return[]        
            noms = [v.strip() for v in x.split(',')] 
            noms_nets = [re.sub(r"\(.*?\)", "", nom).strip() for nom in noms]
            return [n for n in noms_nets if n]
